import { PLATFORM_TYPE, isObject, isString, isFunction, isNumber, capitalize, toCamelCase, internalComponents, isUndefined, noop, isBoolean, hooks, ensure } from '../shared';
...
function updatePropsByPayload(dom, oldProps, updatePayload) {
    ...
    for (let i = 0; i < updatePayload.length; i += 2) {
        ...
        if ("harmony" === PLATFORM_TYPE.HARMONY) {
            if (key === '__fixed') {
                // hack: __fixed最先识别
...
function setEvent(dom, name, value, oldValue) {
    ...
    if (eventName === 'click' && "harmony" !== PLATFORM_TYPE.HARMONY && compName in internalComponents) {
        eventName = 'tap';
    }
    if (isFunction(value)) {
        if (oldValue) {
            dom.removeEventListener(eventName, oldValue, "harmony" !== PLATFORM_TYPE.HARMONY ? false : undefined);
            dom.addEventListener(eventName, value, "harmony" !== PLATFORM_TYPE.HARMONY ? { isCapture, sideEffect: false } : undefined);
        }
        else {
            dom.addEventListener(eventName, value, "harmony" !== PLATFORM_TYPE.HARMONY ? isCapture : undefined);
        }
    }
    else {
...
function setStyle(style, key, value) {
    if (key[0] === '-' && "harmony" !== PLATFORM_TYPE.HARMONY) {
    ...
    style[key] =
        isNumber(value) && IS_NON_DIMENSIONAL.test(key) === false
            ? ("harmony" === PLATFORM_TYPE.HARMONY ? value + 'px' : convertNumber2PX(value))
            : value === null
                ? ''
                : value;
}
        ...
        if (isString(value)) {
            ...
            if (isObject(oldValue)) {
                for (const i in oldValue) {
                    if (!(value && i in value)) {
                        // Harmony特殊处理
                        if ("harmony" === PLATFORM_TYPE.HARMONY && i === 'position' && oldValue[i] === 'fixed') {
                            // @ts-ignore
                            dom.setLayer(0);
                        }
                        setStyle(style, i, '');
            ...
            if (isObject(value)) {
                for (const i in value) {
                    if (!oldValue || !isEqual(value[i], oldValue[i])) {
                        // Harmony特殊处理
                        if ("harmony" === PLATFORM_TYPE.HARMONY && i === 'position') {
                            if (value[i] === 'fixed' || (value[i] !== 'fixed' && (oldValue === null || oldValue === void 0 ? void 0 : oldValue[i]))) {
                                // @ts-ignore
                                dom.setLayer(value[i] === 'fixed' ? 1 : 0);
                            }
